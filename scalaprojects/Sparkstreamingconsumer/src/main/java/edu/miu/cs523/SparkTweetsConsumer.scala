...
object SparkTweetsConsumer {
  ...
  def main(args: Array[String]) {
    ...
    .map(twt=>twt.split("\t")).map(splt=>(splt(0).toLong,splt(1),splt(2).toInt,splt(3),splt(4).toInt,splt(5),splt(6).toInt,splt(7).toBoolean,splt(8).toBoolean,splt(9).toBoolean,splt(10),splt(11)))
 ...
 /*   0 (status.getUser.getId, 
      1 status.getLang,
    	2 status.getUser.getFriendsCount,
      3	status.getUser.getLocation,
    	4 status.getUser.getFollowersCount,
    	5 status.getSource.split("[<,>]")(2),
      6 status.getRetweetCount,
      7 status.isPossiblySensitive,
      8 status.isRetweet,
      9 status.isRetweeted,
     10 status.getCreatedAt.toString,
     11 status.getText) */  
    //tweets.print();
